      ...
      try {
        ...
        const importedContacts: Contact[] = data
          .map((row) => ({
            ...
            "Phone Number":
              row["Phone Number"] || row["number"] || row["phone"] || "",
            "Full Name":
              row["Full Name"] ||
              row["fullName"] ||
              row["Additional Info"] ||
    ...
    if (duplicates.length > 0) {
      if (duplicateMode === "replace") {
        ...
        const duplicateNumbers = new Set(
          duplicates.map((d) => d["Phone Number"]),
        );
        finalContacts = finalContacts.filter(
          (c) => !duplicateNumbers.has(c["Phone Number"]),
        );
      ...
      const action =
        duplicateMode === "replace"
          ...
          : duplicateMode === "keep-both"
            ...
            : "skipped";
      toast.success(
        `Imported ${addedCount} contact${addedCount !== 1 ? "s" : ""}. ${duplicates.length} duplicate${duplicates.length !== 1 ? "s" : ""} ${action}.`,
      );
    } else {
      toast.success(
        `Imported ${addedCount} contact${addedCount !== 1 ? "s" : ""}`,
      );
    }
  };
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>
                Create, edit, and manage your contact lists
              </CardDescription>
            </div>
                    ...
                    <div className="grid gap-2">
                      ...
                      <p className="text-sm text-muted-foreground">
                        ...
                        <code className="bg-muted px-1 py-0.5 rounded">
                          [time] Name (PhoneNumber) - Status
                        </code>
                      </p>
                    </div>
                    <div className="grid gap-2">
                      <Label htmlFor="duplicate-mode">
                        If duplicate phone numbers are found:
                      </Label>
                      <select
                        id="duplicate-mode"
                        value={duplicateMode}
                        onChange={(e) =>
                          setDuplicateMode(
                            e.target.value as "replace" | "keep-both" | "skip",
                          )
                        }
                        className="w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2"
                      >
                        <option value="keep-both">
                          Keep both (add number suffix to name)
                        </option>
                      ...
                      <TableCell className="text-right">
                        <div className="flex justify-end gap-2">
                          <Button
                            ...
                            size="icon"
                            onClick={() =>
                              handleEditClick(contact, startIndex + index)
                            }
                          >
                          ...
                          <Button
                            ...
                            onClick={() =>
                              handleDeleteContact(startIndex + index)
                            }
                          >
