...
import { Search, MessageSquare, Phone, User, UserX, ExternalLink } from "lucide-react"
...
export default function MessageExplorer() {
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-[700px]">
      ...
      <Card className="lg:col-span-1">
        <CardHeader className="pb-4">
          ...
          <CardDescription className="text-base">{conversations.length} contacts with messages</CardDescription>
              ...
              {filteredConversations.map((conversation) => (
                <div
                  ...
                  className={`p-3 rounded-lg cursor-pointer transition-colors ${
                    selectedConversation?.contactName === conversation.contactName ? "bg-muted" : "hover:bg-muted/50"
                  }`}
          ...
          <CardDescription className="text-base">
            {selectedConversation && (
              <div className="flex items-center gap-4">
                ...
                <span>
                  First activity: {new Date(selectedConversation.messages[0]?.Timestamp).toLocaleDateString()} - Last
                  activity: {new Date(selectedConversation.lastActivity).toLocaleDateString()}
                </span>
              </div>
            )}
          </CardDescription>
            ...
            <div className="space-y-4 p-4 pb-2">
              {selectedConversation ? (
                selectedConversation.messages.map((message, index) => (
                  <div key={index} className={`flex gap-3 ${message.isOutgoing ? "flex-row-reverse" : "flex-row"}`}>
                    <div
                      className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${
                        message.isOutgoing ? "bg-blue-500 text-white" : "bg-gray-200 text-gray-700"
                      }`}
                    >
                    ...
                    <div
                      className={`max-w-[85%] rounded-lg p-3 ${
                        message.isOutgoing ? "bg-blue-500 text-white" : "bg-gray-100 text-gray-900"
                      }`}
                    >
                      <div className="flex items-center gap-2 mb-1">
                        <Badge
                          variant="secondary"
                          className={`text-xs ${
                            message.isOutgoing ? "bg-blue-400 text-white" : "bg-gray-200 text-gray-700"
                          }`}
                        >
                          {message.isSMS ? "SMS" : "Call"}
                        </Badge>
                        <span className="text-xs opacity-70">{new Date(message.Timestamp).toLocaleString()}</span>
                      </div>
                      ...
                      {message.isSMS ? (
                        ...
                        <div className="text-sm">
                          <p>
                            {message.isOutgoing ? "Outgoing call" : "Incoming call"} • {message.Type}
                          </p>
                        </div>
                      )}
                    </div>
