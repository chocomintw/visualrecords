        ...
        <TabsContent value="contacts">
          <Card>
            ...
            <CardContent>
              <Table>
                <TableCaption>
                {contactInteractions.length} known contacts with interactions • {contacts.length} total contacts in address book
                </TableCaption>
                <TableHeader>
                  <TableRow>
        ...
        <TabsContent value="unknown">
          <Card>
            ...
            <CardContent>
              <Table>
                <TableCaption>
                  {unknownNumberInteractions.length} unknown numbers with interactions
                </TableCaption>
                <TableHeader>
                  <TableRow>
                    <TableHead>Phone Number</TableHead>
...
function ActivityLevelBadge({ count }: { count: number }) {
  if (count === 0) {
    ...
    return <Badge variant="outline" className="bg-red-50 text-red-700 border-red-200">Very High</Badge>;
  }
}
...
function InteractionTypeBadge({ texts, calls }: { texts: number; calls: number }) {
  ...
  if (textPercentage > 80) {
    return <Badge variant="outline" className="bg-blue-50 text-blue-700 border-blue-200">Mostly Texts</Badge>;
  } else if (callPercentage > 80) {
    return <Badge variant="outline" className="bg-green-50 text-green-700 border-green-200">Mostly Calls</Badge>;
  } else if (texts > calls) {
    return <Badge variant="outline" className="bg-indigo-50 text-indigo-700 border-indigo-200">More Texts</Badge>;
  } else if (calls > texts) {
    return <Badge variant="outline" className="bg-emerald-50 text-emerald-700 border-emerald-200">More Calls</Badge>;
  } else {
    return <Badge variant="outline" className="bg-purple-50 text-purple-700 border-purple-200">Balanced</Badge>;
  }
}

function UnknownNumberStatus({ total }: { total: number }) {
  if (total === 1) {
    return <Badge variant="outline" className="bg-gray-50">One-off</Badge>;
  } else if (total <= 3) {
    return <Badge variant="outline" className="bg-blue-50 text-blue-700 border-blue-200">Occasional</Badge>;
  } else if (total <= 10) {
    return <Badge variant="outline" className="bg-orange-50 text-orange-700 border-orange-200">Frequent</Badge>;
  } else {
    return <Badge variant="outline" className="bg-red-50 text-red-700 border-red-200">Very Frequent</Badge>;
  }
}
